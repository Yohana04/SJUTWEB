<x-app-layout>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6 px-6 py-4 bg-white border border-slate-100 rounded-xl shadow-sm">
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.announcements.create') }}" class="inline-flex items-center gap-2 bg-[#1a3a63] text-white px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-[#2B579A] transition-all shadow active:scale-95">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                New Notice
            </a>
            <form action="{{ route('admin.announcements.import') }}" method="POST" enctype="multipart/form-data" class="inline" id="ann-import-form">
                @csrf
                <input type="file" name="csv_file" id="ann_csv_file" class="hidden" onchange="document.getElementById('ann-import-form').submit()">
                <button type="button" onclick="document.getElementById('ann_csv_file').click()" class="inline-flex items-center gap-2 bg-slate-50 border border-slate-200 text-slate-600 px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-[#2B579A] hover:text-white hover:border-[#2B579A] transition-all active:scale-95">
        ...
        <a href="{{ route('admin.announcements.template') }}" class="inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-[#2B579A] transition-colors">
        ...
        <div class="bg-white border border-slate-100 shadow-sm overflow-hidden animate-fade-in-up">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Type</th>
                        <th class="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none text-center">Status</th>
                        <th class="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Published</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors group">
                    <td class="px-6 py-4">
                        <div class="text-xs font-bold text-slate-700 leading-tight">{{ $announcement->title }}</div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-1.5 font-sans">
                            <div class="w-1 h-1 rounded-full {{ $announcement->status == 'active' ? 'bg-green-500' : 'bg-slate-300' }}"></div>
                            <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest">{{ $announcement->status }}</span>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-1.5">
                            <a href="{{ route('announcements.show', $announcement) }}" target="_blank" title="View Publicly" 
                            ...
                            <a href="{{ route('admin.announcements.edit', $announcement) }}" title="Edit Record"
                               class="p-1.5 bg-slate-100 text-slate-400 hover:bg-[#2B579A]/10 hover:text-[#2B579A] rounded-sm transition-all grayscale hover:grayscale-0">
                            ...
                            <form action="{{ route('admin.announcements.destroy', $announcement) }}" method="POST" onsubmit="return confirm('Archive this notice permanently?')" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="5" class="px-8 py-20 text-center">
                        ...
                        <div class="text-xs font-black text-slate-300 uppercase tracking-[0.2em]">No announcements registered in the system.</div>
                    </td>
                </tr>
                @endforelse
            </tbody>
