<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white border border-slate-100 shadow-xl p-12 animate-fade-in-up">
            <form action="{{ route('admin.departments.update', $department) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Department Name</label>
                    <input type="text" name="name" required value="{{ $department->name }}"
                ...
                <div class="grid grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="text" name="head_of_department" value="{{ $department->head_of_department }}"
                               class="w-full px-6 py-4 bg-slate-50 border border-slate-100 focus:border-[#2B579A] focus:bg-white outline-none transition-all text-sm font-bold">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ $department->phone }}"
                           class="w-full px-6 py-4 bg-slate-50 border border-slate-100 focus:border-[#2B579A] focus:bg-white outline-none transition-all text-sm font-bold">
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="6" 
                              class="w-full px-6 py-4 bg-slate-50 border border-slate-100 focus:border-[#2B579A] focus:bg-white outline-none transition-all text-sm font-medium leading-relaxed">{{ $department->description }}</textarea>
                </div>

                <div class="pt-8 flex justify-end gap-6">
                    <a href="{{ route('admin.departments.index') }}" class="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors">Discard Changes</a>
